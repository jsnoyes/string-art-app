...
function createGrayScale(imgData: ImageData): Uint8ClampedArray {
  const resultArr = new Uint8ClampedArray(imgData.data.length / 4);
...
function createBuffers(pinCoords: Point[], nPins: number, minDistance: number): Map<string, Point[]> {
...
function App() {
  ...
  // Process Image when it's uploaded
  useEffect(() => {
    if (!image) return;
  ...
  const processData = async (img: HTMLImageElement) => {
    ...
    const grayImg = createGrayScale(imgData);
  ...
  async function lineSequenceCalculation(
    ...
    while (withinErrorThreshold) {
      ...
      updateErrorArray(points, error, lineWeight, dimension);
      lastPinsArrInx = updateLastPins(bestPin, lastPinsArrInx, lastPinsArr, lastPinsSet, MIN_LOOP);
      ...
      paint(bestPin, pin, pinCoords, resCtx, scaleValue);
      pin = bestPin;
  ...
  return (
    <div>
      ...
      {image && <img src={image.src} alt="Uploaded preview" style={{ maxWidth: '100%' }} />}
      {resultImage && <img src={resultImage} alt="Processed preview" style={{ maxWidth: '100%' }} />}
